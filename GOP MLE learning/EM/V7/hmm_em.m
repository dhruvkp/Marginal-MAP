...
%                    training instances.
%     parameters: set of parameters (max iteration number, tolerance)
%     X: ( 1 * N Data vector)
%     
% Output:
...
while i<parameters.iterations || abs(llh_old - llh_new) > parameters.tolerance
    ...
    model.A = temp_model.A / size(training_data,1);
    model.B = temp_model.B / size(training_data,1);
    model.S = temp_model.S / size(training_data,1);
