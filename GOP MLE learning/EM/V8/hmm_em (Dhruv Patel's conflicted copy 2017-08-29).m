...
while i<parameters.iterations || abs(llh_old - llh_new) > parameters.tolerance
    ...
    model.A = model.A +  parameters.LR* temp_model.A / size(training_data,1);
    model.B = model.B + parameters.LR* temp_model.B / size(training_data,1);
    model.S = model.S + parameters.LR* temp_model.S / size(training_data,1);
