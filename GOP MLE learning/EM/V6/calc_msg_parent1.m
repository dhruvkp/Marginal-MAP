function [node_list input_tree]=calc_msg_parent1(input_tree, node_no, theta, fixed_nodes,indexes)
...
for j=1:size(parent_list,2)
    ...
    for xj=1:input_tree{current_parent,3}
        ...
        for xi=1:input_tree{node_no,3}
            ...
            if ismember(current_parent,indexes.hidden) && ismember(node_no,indexes.input)
                sum=sum+(exp(theta.h(xj,xi))*message_mult);
                
            elseif ismember(current_parent,indexes.hidden) && ismember(node_no,indexes.hidden)
                sum=sum+(exp(theta.h_to_h(xj,xi))*message_mult);
                
            else
                sum=sum+(exp(theta.h_to_y(xj,xi))*message_mult);
