function [theta_i,theta_c_n] = learn( A,input,output,hidden,x_domain,training_data,default_lambda,solver)
...
[dl_i,dl_c,dlp_i,dlp_c,dl_theta_i,dl_theta_c] = calc_dl_v2( b_i,b_c,bp_i,bp_c,theta_i,theta_c,w_i,w_i_c,sum_b_i,sum_b_c,lambda,cliques,order_variables,hidden,output,-1);
...
%         [bps_i,bps_c,sps_i,sps_c,fvalp_qpbo]=qpbo_update_v2(m,gamma,dlps_i,dlps_c,bps_i,bps_c,cliques,domain_size,number_variables,number_edges,tolearn,number_tolearn_edges,training_data(m,:));
    ...
    [dln_i,dln_c,dlpn_i,dlpn_c,dl_theta_i,dl_theta_c] = calc_dl_v2( b_i,b_c,bp_i,bp_c,theta_i,theta_c,w_i,w_i_c,sum_b_i,sum_b_c,lambda,cliques,order_variables,hidden,output,m);
    dl_i(m,:,:)=dln_i(m,:,:);
    dlp_i(m,:,:)=dlpn_i(m,:,:);
    dl_c(m,:,:,:)=dln_c(m,:,:,:);
    dlp_c(m,:,:,:)=dlpn_c(m,:,:,:);
...
theta_c_n=zeros(number_variables,number_variables,domain_size,domain_size);
