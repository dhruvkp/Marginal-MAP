...
degree=zeros(number_variables,1);
for i=1:number_variables
    degree(i)=sum(find(A(i,:)));
end
...
[dl_i,dl_c,dlp_i,dlp_c,dl_theta_i,dl_theta_c] = calc_dl_v2( b_i,b_c,bp_i,bp_c,theta_i,theta_c,degree,sum_b_i,sum_b_c,lambda,cliques,order_variables,hidden,output,domain_sizes,-1);
...
while true
    ...
    elseif strcmp(solver,'qpbo')
        if length(hidden)>0
            [b_i,b_c,s_i,s_c]=qpbo_update_v2(m,gamma,dl_i,dl_c,b_i,b_c,cliques,2,number_variables,number_edges,hidden,number_hidden_edges,training_data(m,:));
        end
        [bp_i,bp_c,sp_i,sp_c]=qpbo_update_v2(m,gamma,dlp_i,dlp_c,bp_i,bp_c,cliques,2,number_variables,number_edges,tolearn,number_tolearn_edges,training_data(m,:));
    end    
    theta_i=theta_i+step_theta*dl_theta_i;
...
%     
    ...
    [dln_i,dln_c,dlpn_i,dlpn_c,dl_theta_i,dl_theta_c] = calc_dl_v2( b_i,b_c,bp_i,bp_c,theta_i,theta_c,degree,sum_b_i,sum_b_c,lambda,cliques,order_variables,hidden,output,domain_sizes,m);
    dl_i{m}(:,:)=dln_i{m}(:,:);
    dlp_i{m}(:,:)=dlpn_i{m}(:,:);
    for c=1:number_edges
        dl_c{m,c}(:,:)=dln_c{m,c}(:,:);
