function [ dl_i,dl_c,dlp_i,dlp_c ] = calc_dl_v2( b_i,b_c,bp_i,bp_c,w_i,w_i_c,sum_b_i,sum_b_c,lambda,cliques,order_variables,hidden,output,m )
    ...
    for m=1:M
        for i=1:number_variables
            for xi=1:domain_size
                dl_i(m,i,xi)=(b_i(m,i,xi)-bp_i(m,i,xi))/(2*lambda);
                dl_i(m,i,xi)=dl_i(m,i,xi)+sum_b_i(i,xi)/(2*lambda);
                if b_i(m,i,xi)~=0 && ismember(i,hidden)
                    dl_i(m,i,xi)=dl_i(m,i,xi) - w_i(i)- w_i(i)*log(b_i(m,i,xi));
                end
                dlp_i(m,i,xi)=(bp_i(m,i,xi)-b_i(m,i,xi))/(2*lambda);
                dlp_i(m,i,xi)=dlp_i(m,i,xi)-sum_b_i(i,xi)/(2*lambda);
        ...
        for c=1:number_edges
            ...
            for xi=1:domain_size
                for xj=1:domain_size
                    dl_c(m,c,xi,xj)=(b_c(m,c,xi,xj)-bp_c(m,c,xi,xj))/(2*lambda);
                    dl_c(m,c,xi,xj)=dl_c(m,c,xi,xj)+sum_b_c(c,xi,xj)/(2*lambda);
                    dlp_c(m,c,xi,xj)=(bp_c(m,c,xi,xj)-b_c(m,c,xi,xj))/(2*lambda);
                    dlp_c(m,c,xi,xj)=dlp_c(m,c,xi,xj)-sum_b_c(c,xi,xj)/(2*lambda);
