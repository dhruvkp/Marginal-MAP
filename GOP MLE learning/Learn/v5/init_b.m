...
for m=1:M
    ...
    for i=hidden
        b_i(m,i,1:domain_sizes(i))=ones(domain_sizes(i),1)/domain_sizes(i);
        bp_i(m,i,1:domain_sizes(i))=ones(domain_sizes(i),1)/domain_sizes(i);
    end
    for i=output
        b_i(m,i,training_data(m,i))=1;
        bp_i(m,i,1:domain_sizes(i))=ones(domain_sizes(i),1)/domain_sizes(i);
...
for c=1:number_edges
    ...
    elseif ismember(i,input) && ~ismember(j,input)
        if ismember(j,output)
            for m=1:M
                b_c(m,c,training_data(m,i),training_data(m,j))=1;
                bp_c(m,c,training_data(m,i),1:domain_sizes(j))=ones(domain_sizes(j),1)/domain_sizes(j);
            end
        else
            for m=1:M
                b_c(m,c,training_data(m,i),1:domain_sizes(j))=ones(domain_sizes(j),1)/domain_sizes(j);
                bp_c(m,c,training_data(m,i),1:domain_sizes(j))=ones(domain_sizes(j),1)/domain_sizes(j);
            end
        end
    elseif ~ismember(i,input) && ismember(j,input)
        if ismember(i,output)
            for m=1:M
                b_c(m,c,training_data(m,i),training_data(m,j))=1;
                bp_c(m,c,1:domain_sizes(i),training_data(m,j))=ones(domain_sizes(i),1)/domain_sizes(i);
            end
        else
            for m=1:M
                b_c(m,c,1:domain_sizes(i),training_data(m,j))=ones(domain_sizes(i),1)/domain_sizes(i);
                bp_c(m,c,1:domain_sizes(i),training_data(m,j))=ones(domain_sizes(i),1)/domain_sizes(i);
            end
        end
    else
        for m=1:M
            bp_c(m,c,1:domain_sizes(i),1:domain_sizes(j))=ones(domain_sizes(i),domain_sizes(j))/(domain_sizes(i)*domain_sizes(j));
            ...
            elseif ismember(i,hidden) && ismember(j,output)
                ...
                b_c(m,c,1:domain_sizes(i),1:domain_sizes(j))=ones(domain_sizes(i),domain_sizes(j))/(domain_sizes(i)*domain_sizes(j));
            end
        end
    end
end
