function [theta_c_n] = learn( A,input,output,hidden,domain_sizes,training_data,default_lambda,solver)
...
[ Aeq_1,Aeq_2,Beq_1,Beq_2,lb_1,ub_1 ] = linprog_init( A,domain_sizes,cliques );
...
sum_b_c=zeros(number_edges,max_domain_size,max_domain_size);
for i=1:number_variables
...
%         break;
%     end
    if (gap<1 && theta_gap<1 && t>1000) || t>(10^5)
        gap
        theta_gap
        break
    end
    ...
    elseif strcmp(solver,'linprog1')
        if length(hidden)>0
            [b_i,b_c,s_i,s_c]=linprog_update( A,m,cliques,domain_sizes,training_data,b_i,b_c,-dl_i,-dl_c,horzcat(input,output),Aeq_1,Aeq_2,Beq_1,Beq_2,lb_1,ub_1,gamma);
        end
        [bp_i,bp_c,sp_i,sp_c]=linprog_update( A,m,cliques,domain_sizes,training_data,bp_i,bp_c,dlp_i,dlp_c,input,Aeq_1,Aeq_2,Beq_1,Beq_2,lb_1,ub_1,gamma);
