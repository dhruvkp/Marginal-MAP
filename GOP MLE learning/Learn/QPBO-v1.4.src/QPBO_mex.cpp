...
void recoverEdgeBeliefs(Mat<COST_TYPE> &edgeBelief,
  ...
  for (int i, j, e = 0; e < nEdges; e++) {
    ...
    if (xi >= 0 && xj >= 0) {
      edgeBelief(exactIdxs[xi][xj],e) = 1; //00,01,01,11
    } else if (xi < 0 && xj >= 0) {
      edgeBelief(exactIdxs[0][xj],e) = 0.5;
      edgeBelief(exactIdxs[1][xj],e) = 0.5;
    } else if (xi >= 0 && xj < 0) {
      edgeBelief(exactIdxs[xi][0],e) = 0.5;
      edgeBelief(exactIdxs[xi][1],e) = 0.5;
    } else if (xi < 0 && xj < 0) {
...
//         mexPrintf("Edge (%d, %d) beliefs (0/00=%g, 1/01=%g, 2/10=%g, 3/11=%g)\n",
//         i,
//         j,
//         edgeBelief(0, e),
//         edgeBelief(1, e),
