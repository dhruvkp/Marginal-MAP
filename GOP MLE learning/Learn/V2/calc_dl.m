function [ dl_i,dl_c,dl_pa_i,dlc_i,dlc_c,dlc_pa_i ] = calc_dl( b_i,b_c,b_pa_i,bp_i,bp_c,bp_pa_i,w_i,w_i_c,lambda,cliques,order_variables )
...
dlc_i=zeros(size(b_i));
dlc_c=zeros(size(b_c));
dlc_pa_i=zeros(size(b_pa_i));
...
for m=1:M
    for i=1:number_variables
        for xi=1:domain_size
            ...
            dlc_i(m,i,xi)=(bp_i(m,i,xi)-b_i(m,i,xi))/lambda  ;
            if bp_i(m,i,xi)~=0
                dlc_i(m,i,xi)=dlc_i(m,i,xi)+ w_i(i)+ w_i(i)*log(bp_i(m,i,xi));
            end
        end
    end
        ...
        for xi=1:domain_size
            for xj=1:domain_size
                if find(order_variables==i)<find(order_variables==j) % j is parent of i
                    ...
                    dlc_c(m,c,xi,xj)=(bp_c(m,c,xi,xj)-b_c(m,c,xi,xj))/lambda  ;
                    if bp_c(m,c,xi,xj)~=0
                        dlc_c(m,c,xi,xj)=dlc_c(m,c,xi,xj)+ w_i_c(i,c)+ w_i_c(i,c)*log(bp_c(m,c,xi,xj));
                    end
                else
                    dl_c(m,c,xi,xj)=(b_c(m,c,xi,xj)-bp_c(m,c,xi,xj))/lambda ;
                     if b_c(m,c,xi,xj)~=0
                    ...
                    if bp_c(m,c,xi,xj)~=0
                        dlc_c(m,c,xi,xj)=dlc_c(m,c,xi,xj) + w_i_c(j,c)+ w_i_c(j,c)*log(bp_c(m,c,xi,xj));
                    end
                end
            end
        end
    ...
    for c=1:number_edges
        ...
        if find(order_variables==i)<find(order_variables==j) % j is parent of i, j is summed out later than i
            for xj=1:domain_size
                ...
                dlc_pa_i(m,c,xj)=-w_i_c(i,c)+w_i_c(j,c) ;
                if bp_pa_i(m,c,xj)~=0
                    dlc_pa_i(m,c,xj)=dlc_pa_i(m,c,xj)-(w_i_c(i,c)-w_i_c(j,c))*log(bp_pa_i(m,c,xj));
                end
            end
        else
            for xi=1:domain_size
                ...
                dlc_pa_i(m,c,xi)=-w_i_c(j,c)+w_i_c(i,c) -(w_i_c(j,c)-w_i_c(i,c))*log(bp_pa_i(m,c,xi)); 
                if bp_pa_i(m,c,xi)~=0
                    dlc_pa_i(m,c,xi)=dlc_pa_i(m,c,xi)-(w_i_c(j,c)-w_i_c(i,c))*log(bp_pa_i(m,c,xi));
                end
            end
        end
    end
