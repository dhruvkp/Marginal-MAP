function [theta_c] = learn( A,input,output,hidden,x_domain,training_data,iterations)
...
% finding lambda
lambda=max(1./min(w_i),1./min(w_i_c(w_i_c>0)));
% for i=1:number_variables
%     tmp=0;
%     if w_i(i,1)~=0
...
Aeq1=zeros(size(dl,2),number_variables);
...
Aeq2=zeros(size(dl,2),2*domain_size*number_edges);
...
Aeq3=zeros(size(dl,2),domain_size*number_edges);
...
while true
    ...
    [dl_i,dl_c,dl_pa_i,dlp_i,dlp_c,dlp_pa_i] = calc_dl( b_i,b_c,b_pa_i,bp_i,bp_c,bp_pa_i,w_i,w_i_c,lambda,cliques,order_variables );
    
    %%% choose random sample to optimize on %%%
    ...
    Aeq4=zeros(size(dl,2),length(fixed_variables));
    ...
    dl=zeros(1,number_variables*domain_size+number_edges*(domain_size^2)+number_edges*domain_size);
    b_0=zeros(1,number_variables*domain_size+number_edges*(domain_size^2)+number_edges*domain_size);
    cnt=1;
    for i=1:number_variables
        for xi=1:domain_size
            dl(1,cnt)=dl_i(m,i,xi);
            b_0(1,cnt)=b_i(m,i,xi);
            cnt=cnt+1;
        end
    end
    for c=1:number_edges
    ...
    for c=1:number_edges
        for xi=1:domain_size
            ...
            b_0(1,cnt)=b_pa_i(m,c,xi);
            cnt=cnt+1;
        end
    end
    
    % solve linear program for b
    s_t(m,:)=linprog(-dl,[],[],Aeq,Beq,lb,ub,[],optimoptions('linprog','Display','off'));
    ...
    Aeq4=zeros(size(dl,2),length(input));
    ...
    bp_0=zeros(1,number_variables*domain_size+number_edges*(domain_size^2)+number_edges*domain_size);
    cnt=1;
    for i=1:number_variables
        for xi=1:domain_size
            dlp(1,cnt)=dlp_i(m,i,xi);
            bp_0(1,cnt)=bp_i(m,i,xi);
            cnt=cnt+1;
        end
    end
    for c=1:number_edges
    ...
    for c=1:number_edges
        for xi=1:domain_size
            ...
            bp_0(1,cnt)=bp_pa_i(m,c,xi);
            cnt=cnt+1;
        end
    end
    
    % solve linear program for bp
    sp_t(m,:)=linprog(dlp,[],[],Aeq,Beq,lb,ub,[],optimoptions('linprog','Display','off'));
   ...
   gap
        if abs(gap)>abs((8)*(10^(3)))
            gap
            break;
        end
        

%     if abs(gap)<10^20 && t>1000
%         gap
%         t
%         break
%     end
