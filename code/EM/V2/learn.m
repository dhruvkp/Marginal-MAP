...
lambda=calc_lambda( A,input,output,hidden,b_i,b_c,w_i,w_i_c );
...
while true
    ...
    for m=1:M
        ...
        for i=1:number_variables
            for xi=1:domain_size
                gap=gap+abs((b_i(m,i,xi)-s_t(m,cnt))*dl_i(m,i,xi))+abs((bp_i(m,i,xi)-sp_t(m,cnt))*dlp_i(m,i,xi));
                cnt=cnt+1;
            end
        end
        for c=1:number_edges
            for xi=1:domain_size
                for xj=1:domain_size
                    gap=gap+abs((b_c(m,c,xi,xj)-s_t(m,cnt))*dl_c(m,c,xi,xj))+abs((bp_c(m,c,xi,xj)-sp_t(m,cnt))*dlp_c(m,c,xi,xj));
                    cnt=cnt+1;
                end
            end
        end
        for c=1:number_edges
            for xi=1:domain_size
                gap=gap+abs((b_pa_i(m,c,xi)-s_t(m,cnt))*dl_pa_i(m,c,xi))+abs((bp_pa_i(m,c,xi)-sp_t(m,cnt))*dlp_pa_i(m,c,xi));
                cnt=cnt+1;
            end
        end
    end
